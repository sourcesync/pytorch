...
/// Returns true if APU device is available.
bool TORCH_API is_available();

/// Sets the RNG seed for the APU device.
void TORCH_API manual_seed(uint64_t seed);

/// Waits for all streams on the APU device to complete.
...
/// Submits the currently active command buffer to run on the APU device.
